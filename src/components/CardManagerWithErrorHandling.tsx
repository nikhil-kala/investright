...
// Try to import WebSearchService, fallback to mock if it fails
...
try {
  ...
  console.error('❌ Error loading WebSearchService, using mock:', error);
  webSearchService = new MockWebSearchService();
}

export default function CardManagerWithErrorHandling() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      // Use the webSearchService with error handling
      const results = await webSearchService.searchCreditCards(searchTerm, (progress: SearchProgress) => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Credit Card Manager (With Error Handling)</h1>
        <p className="text-gray-600">Testing with error handling for WebSearchService</p>
      </div>
          ...
          <div className="mb-4 p-4 bg-blue-50 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-blue-900">
                {searchProgress.status === 'searching' && `Searching ${searchProgress.currentBank}...`}
                {searchProgress.status === 'parsing' && 'Parsing results...'}
                {searchProgress.status === 'completed' && 'Search completed!'}
                {searchProgress.status === 'error' && 'Search failed'}
              </span>
